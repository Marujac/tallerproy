...
import { getUserHistory, addUserHistory } from '@/lib/demo-store';
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { error: 'Ocurrió un error inesperado al obtener el historial.' },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json(
      { error: 'Ocurrió un error inesperado al guardar el historial.' },
      { status: 500 }
    );
  }
}
